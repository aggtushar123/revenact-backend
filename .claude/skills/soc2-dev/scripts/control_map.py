...
IGNORE_DIRS = {".git", "node_modules", "vendor", "dist", "build", "target", "__pycache__", ".venv", "venv", ".terraform", ".soc2", ".claude"}
ANNOT = re.compile(r"SOC2:([A-Z]+-\d{2})\b")
ROW = re.compile(r"^\|\s*([A-Z]+-\d{2})\s*\|\s*([a-z/]+)\s*\|\s*([^|]*)\|")
PATH_IN_CELL = re.compile(r"`([^`:\s]+?\.[A-Za-z0-9]+)(?::[^`]*)?`")

# Extensions that make a backticked token a file reference rather than a symbol
# (`user.deactivate`, `core.AuditEvent`) or a host path (`/opt/app/deploy.log`).
CODE_EXTS = {
    ".py", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".go", ".java", ".kt", ".rb", ".cs", ".php",
    ".tf", ".tftpl", ".yml", ".yaml", ".json", ".toml", ".sh", ".md", ".env", ".txt", ".cfg",
    ".ini", ".sql", ".html", ".css",
}
_BASENAMES: dict[str, set] = {}


def _missing(root: str, pth: str) -> bool:
    """True when `pth` names a file in this repo that no longer exists. Not reported:
    tokens without a code extension, absolute host paths, references into another
    repository (first segment is not a directory here), and bare basenames that
    exist somewhere in the tree (`ci.yml` for `.github/workflows/ci.yml`)."""
    if os.path.splitext(pth)[1].lower() not in CODE_EXTS or pth.startswith("/"):
        return False
    if os.path.exists(os.path.join(root, pth)):
        return False
    if "/" in pth:
        return os.path.isdir(os.path.join(root, pth.split("/", 1)[0]))
    if root not in _BASENAMES:
        names: set[str] = set()
        for dn, dirs, files in os.walk(root):
            dirs[:] = [d for d in dirs if d not in IGNORE_DIRS or d == ".soc2"]
            names.update(files)
        _BASENAMES[root] = names
    return pth not in _BASENAMES[root]
