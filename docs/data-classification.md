...
| `attention.AttentionSnooze` | internal | — | a per-user snooze on a Dashboard Overview "Needs attention" item; `key` and `fingerprint` reference an item by field values only, no record text |
