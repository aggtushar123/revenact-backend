...
retrieved content — not RAG in the embeddings/semantic-search sense, no
vector DB yet: `find_mentioned_company` is a plain case-insensitive
substring match of the question against the caller's own company names;
a match gets its own recent real Emails (subject+body)/Notes
(title+body)/open Tickets (title only — the model has no body
field)/Activities (a categorical type label, no free text) pulled in
full via `retrieve_recent_communications`. No company named falls back
to a smaller slice for each of the digest's own top-3 at-risk customers,
so the digest isn't pure numbers even then. Every source here is real
data that already backs the Activity Feed's own Emails/Notes/Tickets/
Activities tabs — nothing new is captured, just retrieved differently.
