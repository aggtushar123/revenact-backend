...
| `views.py: AccountStatsView` | `GET /api/v1/accounts/stats/` — Health/NPS/Lifecycle rollups across every Account the tenant has, same shape as `CustomerStatsView`. Backs the standalone Accounts page's own MetricsPanel |
...
| `views.py: CustomerContactListView` | `GET/POST /customers/<customer_id>/contacts/` — GET rolls up both org-level contacts on this Customer *and* account-level contacts on any of its Accounts (see Contact's own docstring); POST always adds an org-level one, `customer` set from the URL |
...
| `models.py: Opportunity` | Same exactly-one-parent shape as `Contact` — a sales opportunity moving through the standalone Pipelines board's "Opportunities" tab. `stage` is a fixed 6-value enum matching the board's own Kanban columns (no per-tenant pipeline customisation). Full CRUD. |
| `views.py: CustomerOpportunityListView` / `AccountOpportunityListView` | `GET/POST /customers/<customer_id>/opportunities/` (rolls up org+account levels, same as Contact) and `.../accounts/<account_id>/opportunities/` (one Account's own) |
| `views.py: OpportunityListView` / `OpportunityDetailView` | `GET/POST /api/v1/opportunities/` (unpaginated — a board needs every card, unlike Contact's own paginated list; POST takes `customer_id` or `account_id` in the body) and `GET/PATCH/DELETE /api/v1/opportunities/<id>/` (flat, powers drag-and-drop + Edit/Delete) |
| `management/commands/seed_demo_opportunities.py` | Dev-only: seeds Opportunity rows under every seeded Customer/Account, spread across all 6 stages. Idempotent. |
| `models.py: Risk` | Same exactly-one-parent shape as `Opportunity` — a customer-health risk tracked on the standalone Pipelines board's "Risks" tab. `stage` is a fixed 4-value enum matching the board's own Kanban columns (Open/Mitigated/Realised/Abandoned). Full CRUD. |
| `views.py: CustomerRiskListView` / `AccountRiskListView` | `GET/POST /customers/<customer_id>/risks/` (rolls up org+account levels, same as Opportunity) and `.../accounts/<account_id>/risks/` (one Account's own) |
| `views.py: RiskListView` / `RiskDetailView` | `GET/POST /api/v1/risks/` (unpaginated, same reasoning as Opportunity's own; POST takes `customer_id` or `account_id` in the body) and `GET/PATCH/DELETE /api/v1/risks/<id>/` (flat, powers drag-and-drop + Edit/Delete) |
| `management/commands/seed_demo_risks.py` | Dev-only: seeds Risk rows under every seeded Customer/Account, spread across all 4 stages. New demo content, not lifted from the mock (the mock's own Risk cards named companies that don't exist in seed data). Idempotent. |
